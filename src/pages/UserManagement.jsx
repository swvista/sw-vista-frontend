...
const UserDetailView = ({
  ...
  const [initialEffectivePermissions, setInitialEffectivePermissions] =
    useState(new Set());
  ...
  useEffect(() => {
    if (formData && initialFormData) {
      ...
      const permissionsChanged =
        JSON.stringify(Array.from(userEffectivePermissions).sort()) !==
        JSON.stringify(Array.from(initialEffectivePermissions).sort());
  ...
  const handleClubChange = (clubId) => {
    setFormData((prev) => ({
      ...prev,
      profile: { ...(prev.profile || {}), club: clubId },
    }));
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const fullNewName = `${formData.first_name || ""} ${
        formData.last_name || ""
      }`.trim();
  ...
  const visibleProfileFields = useMemo(() => {
    ...
    if (!formData || !formData.groups || !allRoles.length || !categoryFields)
      return [];

    const userRoleNames = formData.groups
      .map((groupId) => {
        const role = allRoles.find((r) => r.id === groupId);
                ...
                <div className="col-span-2 flex items-center space-x-4">
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        handleCheckboxChange("is_staff", checked)
                      }
                    />
              ...
              <div className="grid grid-cols-3 items-center gap-4 pt-4 border-t">
                ...
                <p className="col-span-2 text-sm text-gray-500">
                  {formData.date_joined
                    ? new Date(formData.date_joined).toLocaleString()
                    : "N/A"}
                </p>
              </div>
              <div className="grid grid-cols-3 items-center gap-4">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mb-4">
                  Permissions are inherited from the roles above. Changing roles
            ...
            {formData.profile ? (
              <div className="space-y-4 max-w-md">
                {visibleProfileFields.length > 0 ? (
                  visibleProfileFields.map((fieldKey) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Select
                            ...
                            value={formData.profile.club?.toString() || ""}
                          >
                    ...
                    return (
                      <DetailInput
                        ...
                        label={config.label}
                        name={fieldKey}
                        value={formData.profile[fieldKey]}
                        source="profile"
                      />
                    );
                  })
                  ...
                  <p className="text-gray-500">
                    No specific profile information for the selected role(s).
                  </p>
                )}
              </div>
            ) : (
              <p className="text-gray-500">
                This user does not have an extended profile.
              </p>
            )}
          </TabsContent>
        </Tabs>
...
const RoleDetailView = ({ role, allPermissions, onBack, onRoleUpdate }) => {
  ...
  useEffect(() => {
    const permissionsChanged =
      JSON.stringify(Array.from(currentPermissions).sort()) !==
      JSON.stringify(Array.from(initialPermissions).sort());
...
export default function UserManagement() {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      const roleMatch =
        roleFilter === "all" ||
        (user.groups && user.groups.includes(parseInt(roleFilter)));
  ...
  const fetchData = useCallback(async () => {
    try {
      const [meRes, usersRes, rolesRes, permissionsRes, clubsRes] =
        await Promise.all([
          getME(),
          getAllUsers(),
          getAllGroups(),
    ...
    return categoryFields[formCategory].map((field) => {
      ...
      return (
        <div className="space-y-2" key={field}>
          <Label htmlFor={field}>{fieldConfig[field].label}</Label>
          <Input
            id={field}
            name={field}
            value={userForm[field] || ""}
        ...
        <SheetContent className="max-w-md w-full px-5">
          ...
          <form onSubmit={handleUserCreateSubmit} className="space-y-4 mt-4">
            ...
            <MultiSelect
              ...
              onChange={(selected) =>
                setUserForm({ ...userForm, groups: selected })
              }
              placeholder="Select roles"
            />
            <Button type="submit" className="w-full">
