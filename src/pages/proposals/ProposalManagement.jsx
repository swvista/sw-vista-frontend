  ...
  return (
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent className="max-w-md w-full px-5">
        <SheetHeader>
          <SheetTitle>
            {proposal?.id ? "Edit Proposal" : "Create Proposal"}
          </SheetTitle>
        </SheetHeader>
      ...
      <CardContent className="pt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm mb-6">
          <div>
            ...
            <p>{proposal.user?.username || "N/A"}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className={getStatusInfo(proposal.status).color}>
              {getStatusInfo(proposal.status).text}
            </p>
          </div>
          <div>
          ...
          <div className="flex flex-col gap-4 pt-4 border-t">
            ...
            {showComments && (
              <div className="flex flex-col gap-2">
                ...
                <Button
                  variant="destructive"
                  onClick={() => onReject(proposal.id, comments)}
                >
  ...
  const handleApprove = async (proposalId) => {
    if (window.confirm("Are you sure you want to approve this proposal?")) {
      try {
        ...
        toast.error(
          error.response?.data?.detail || "Failed to approve proposal."
        );
      }
    }
  };
  ...
  const handleReject = async (proposalId, comments) => {
    if (window.confirm("Are you sure you want to reject this proposal?")) {
      try {
        ...
        toast.error(
          error.response?.data?.status || "Failed to reject proposal."
        );
      }
    }
  };
